...
TEST(Transaction, signWithLeadingZeroes) {
  ...
  std::size_t transactionLength = Transaction::sign(&values, privateKey, transaction);

  ASSERT_EQ(transactionLength, 113);
...
TEST(Transaction, signWith63bitR) {
  ...
  ASSERT_EQ(transactionLength, 108);
...
TEST(Transaction, signWith62bitRS) {
  ...
  ASSERT_EQ(transactionLength, 95);
