...
namespace RLP
{
   ...
   * @return output buffer length
   */
  inline std::size_t encodeList(Item input[], std::size_t inputLength, Buffer output, bool withSignature)
  {
    std::size_t payloadLength = 0;
    for (std::size_t i = 0; i < (withSignature ? inputLength : inputLength - 3); i++)
    {
      // Shift output buffer by 9 as it is maximum encodeLength length
      payloadLength += encodeItem(input + i, output + payloadLength + 9);

      // Veeery hacky way to inject empty access list (EIP1559 compliant)
      if (i == 7)
      {
        *(output + payloadLength + 9) = 0xc0;
        payloadLength += 1;
      }
    }
